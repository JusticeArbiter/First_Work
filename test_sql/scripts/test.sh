...
#echo LOG_PASS_DIR=$LOG_PASS_DIR
#echo LOG_DIFFERENT_DIR=$LOG_DIFFERENT_DIR
#echo LOG_FILERROR_DIR=$LOG_FILERROR_DIR
#echo LOG_OTHER_DIR=$LOG_OTHER_DIR
...
#  then
#    echo -e "Please to the $TEST_DIR/scripts/env.sh check environment variables\t.............no"
#    exit 1
#fi
#	echo
...
#  else
#    echo -e "$PGHOME/bin/initdb --data-encryption pgcrypto --key-url ldaps://192.168.100.181:636?cn=hgdb,dc=highgo,dc=com?123 -C $jname --new-key -D $PGHOME/data" >> $RESULTS_DIR/initdb.log 2>&1
#    $PGHOME/bin/initdb --data-encryption pgcrypto --key-url ldaps://192.168.100.181:636?cn=hgdb,dc=highgo,dc=com?123 -C $jname --new-key -D $PGHOME/data >> $RESULTS_DIR/initdb.log 2>&1
    #echo -e "Please enter an initialization parameter ID\t.............no"
#fi
...
if [ `ls $SQL_DIR/ | wc -l` -eq `ls $EXPECTED_DIR/ | wc -l` ]
  then
    ...
    for dir_name in `ls $SQL_DIR | sort -n`
    do
      #检查每个文件夹下的文件数量
      if [ `ls $SQL_DIR/$dir_name/ | wc -l` -eq `ls $EXPECTED_DIR/$dir_name/ | wc -l` ]
        then
          echo -e "The directory $SQL_DIR/$dir_name and $EXPECTED_DIR/$dir_name have same number of files\t.............ok" | output_terminal
          #检查每个文件夹下文件的名字对应
          for sql_name in `ls $EXPECTED_DIR/$dir_name/ | sort -n`
          do
            sql_newname=${sql_name%.exp}
            if [ -e $SQL_DIR/$dir_name/${sql_newname}.sql -a -f $SQL_DIR/$dir_name/${sql_newname}.sql ]
              then
                echo -e "$EXPECTED_DIR/$dir_name/$sql_name and $SQL_DIR/$dir_name/${sql_newname}.sql\t.............ok" | output_terminal
              else
                echo -e "Please check $EXPECTED_DIR/$dir2_name/$sql_name and $OUT_DIR/$dir_name/${sql_newname}.out\t.............no"
                exit 1
            fi
          done
        else
          echo -e "Please check the directory $SQL_DIR/$dir_name and $EXPECTED_DIR/$dir_name,number of files are different\t.............no"
          exit 1
      fi
    done
  else
...
for dir2_name in `ls $SQL_DIR/ | sort -n`
do
  mkdir $OUT_DIR/$dir2_name
  for sql2_name in `ls $SQL_DIR/$dir2_name | sort -n`
  do
    out_sqlname=${sql2_name%.sql}
    $PGHOME/bin/psql -h $PGHOST -p $PGPORT -U $PGUSER -d $PGDATABASE -a -f $SQL_DIR/$dir2_name/$sql2_name 2>&1 | sed 's/^psql.*ERROR/ERROR/g' 2>&1 | sed 's/^psql.*WARNING/WARNING/g' 2>&1 | sed 's/^psql.*NOTICE/NOTICE/g' 2>&1 | sed 's/^psql.*invalid/invalid/g' | sed 's/^psql.*INFO/INFO/g' > $OUT_DIR/$dir2_name/${out_sqlname}.out 2>&1
    #检查输出文件是否输出到了指定的文件夹
    if [ -e $OUT_DIR/$dir2_name/${out_sqlname}.out -a -f $OUT_DIR/$dir2_name/${out_sqlname}.out ]
      then
        echo -e "$SQL_DIR/$dir2_name/$sql2_name output to $OUT_DIR/$dir2_name/${out_sqlname}.out\t.............ok" | output_terminal
    else
      echo -e "Please check the $SQL_DIR/$dir2_name/$sql2_name and $OUT_DIR/$dir2_name/${out_sqlname}.out\t.............no"
      exit 1
    fi
  done
done
...
echo -e "Check directory $OUT_DIR and $EXPECTED_DIR\t............."
...
if [ `ls $OUT_DIR/ | wc -l` -eq `ls $EXPECTED_DIR/ | wc -l` ]
  then
    ...
    for dir3_name in `ls $SQL_DIR | sort -n`
    do
      #检查每个文件夹下的文件名字的对应
      if [ `ls $OUT_DIR/$dir3_name/ | wc -l` -eq `ls $EXPECTED_DIR/$dir3_name/ | wc -l` ]
        then
          echo -e "The directory $OUT_DIR/$dir3_name and $EXPECTED_DIR/$dir3_name have same number of files\t.............ok" | output_terminal
          #检查文件夹下文件名
          for sql3_name in `ls $EXPECTED_DIR/$dir3_name/ | sort -n`
          do
            sql3_newname=${sql3_name%.exp}
            if [ -e $OUT_DIR/$dir3_name/${sql3_newname}.out -a -f $OUT_DIR/$dir3_name/${sql3_newname}.out ]
              then
                echo -e "$EXPECTED_DIR/$dir3_name/$sql3_name and $OUT_DIR/$dir3_name/${sql3_newname}.out\t.............ok" | output_terminal
              else
                echo -e "Please check $EXPECTED_DIR/$dir3_name/$sql3_name and $OUT_DIR/$dir3_name/${sql3_newname}.out\t.............no"
                exit 1
            fi
          done
        else
          echo -e "Please check the directory $OUT_DIR/$dir3_name and $EXPECTED_DIR/$dir3_name,number of files are different\t.............no"
          exit 1
      fi
    done
  else
...
echo -e "The directory $OUT_DIR and $EXPECTED_DIR\t.............ok"
...
oth=0
#逐个文件夹的对比
for dir4_name in `ls $OUT_DIR/ | sort -n`
do
  #逐个文件的对比
  for sql4_name in `ls $OUT_DIR/$dir4_name | sort -n`
  do
    let sum+=1
    name=${sql4_name%.out}
    diff $OUT_DIR/$dir4_name/$sql4_name $EXPECTED_DIR/$dir4_name/${name}.exp > $LOG_DIFFERENT_DIR/tmp.dif 2>&1
    return_id=`echo $?`
    #如果diff返回值为0，则测试通过，输入结果到pass
    if [ $return_id -eq 0 ]
      then
        let pas+=1
        echo -e "The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............ok" | output_terminal
        echo -e "The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............ok" >> $LOG_PASS_DIR/pass.log 2>&1
    #如果diff返回值为1，则未通过测试，输入结果到different
    elif [ $return_id -eq 1 ]
      then
        let dif+=1
        echo -e "The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............no" | output_terminal
        echo -e "The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............no" >> $LOG_DIFFERENT_DIR/different.log 2>&1
        mkdir $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name}
        if [ -e $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name} -a -d $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name} ]
        then
          #把实际输出文件、期望输出文件和对比结果文件放入单独创建的文件夹下
          cp $OUT_DIR/$dir4_name/$sql4_name $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name}/
          cp $EXPECTED_DIR/$dir4_name/${name}.exp $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name}/
          cat $LOG_DIFFERENT_DIR/tmp.dif > $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name}/${name}.dif 2>&1
          #把实际输出和期望输出目录、造成对比结果不一致的sql语句、对比结果追加到一个单独的文件中
          echo -e "$OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\n" >> $LOG_DIFFERENT_DIR/different_infor.log 2>&1
          cat $SQL_DIR/$dir4_name/${name}.sql >> $LOG_DIFFERENT_DIR/different_infor.log 2>&1
          cat $LOG_DIFFERENT_DIR/tmp.dif >> $LOG_DIFFERENT_DIR/different_infor.log 2>&1
          echo -e "\n" >> $LOG_DIFFERENT_DIR/different_infor.log 2>&1
          echo -e "*****************************************************" >> $LOG_DIFFERENT_DIR/different_infor.log 2>&1
        else
          echo -e "Failed to create $LOG_DIFFERENT_DIR/${dir4_name}_${sql4_name},please to check\t.............no"
          exit 1
        fi
    #如果diff返回值为2，则实际输出文件和期望输入文件存在丢失或被损坏情况，输出结果到filerroe中
      ...
      then
        let err+=1
        echo -e "The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............no"
        echo -e "Please check The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp\t .............no\n" >> $LOG_FILERROR_DIR/error_file.log 2>&1
    #如果diff返回值不正常，则可能是其它原因造成，立即退出测试
    else     
      let oth+=1
      echo -e "The diff have some questions on The $OUT_DIR/$dir4_name/$sql4_name and $EXPECTED_DIR/$dir4_name/${name}.exp" >> $LOG_OTHER_DIR/other_questions.log 2>&1
   fi
  done
done
echo
