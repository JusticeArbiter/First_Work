...
#删除双机热备文件夹下的所有文件
echo -e "Empty the directory $DOUBLE_ASYNC_DIR\t............."
rm -rf $DOUBLE_ASYNC_DIR/*
empty_dir_check $DOUBLE_ASYNC_DIR

#创建双机热备项目下的各个文件夹
echo -e "Create the directory\t............."
mkdir $DOUBLE_ASYNC_DIR/log
mkdir $DOUBLE_ASYNC_DIR/data_backup
#检查目录是否创建成功
check_dir $DOUBLE_ASYNC_DIR/log $DOUBLE_ASYNC_DIR/data_backup

#操作备服务器
echo -e "Configuration the standby server\t............."
#在备服务器创建文件夹
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh $STA_SYSTEM_USER@$STADB_HOST mkdir $STADB_HOME/double_check"
#把需要在备服务器执行的脚本scp到备服务器新建的文件夹下
output_log $DOUBLE_ASYNC_DIR/log/log.log "scp -v $DOUBLE_STASH_DIR/* $STA_SYSTEM_USER@$STADB_HOST:$STADB_HOME/double_check/"
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh $STA_SYSTEM_USER@$STADB_HOST chmod u+x $STADB_HOME/double_check/*"
#执行测试前的数据库服务检查，备份data
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh $STA_SYSTEM_USER@$STADB_HOST export PGHOME=$STADB_HOME;export PGDATA=$STADB_HOME/data;$STADB_HOME/double_check/dbservice_check_sta_01.sh"
#删除备库data
...
daname_ba=doublewal_async

#开始测试前数据库状态检查，备份原data
dbservice_check_01 $DOUBLE_ASYNC_DIR/log/log.log

#删除data目录
echo -e "Delete the directory $PGHOME/data\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "rm -rf $PGHOME/data"
delete_dir_check $PGHOME/data

#初始化数据库
echo -e "Init the HighGo Database\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "initdb_stat"
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "mv $PGHOME/data/pg_hba.conf $PGHOME/data/pg_hba.conf.back"
output_log $DOUBLE_ASYNC_DIR/log/log.log "cp $PGHOME/data/postgresql.conf $PGHOME/data/postgresql.conf.back"
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e local all all trust >> $PGHOME/data/pg_hba.conf 2>&1"

echo -e "host all all 127.0.0.1/32 trust" >> $PGHOME/data/pg_hba.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e host all all 127.0.0.1/32 trust >> $PGHOME/data/pg_hba.conf 2>&1"

echo -e "host all all 0.0.0.0/0 trust" >> $PGHOME/data/pg_hba.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e host all all 0.0.0.0/0 trust >> $PGHOME/data/pg_hba.conf 2>&1"

echo -e "host replication all 0.0.0.0/0 trust" >> $PGHOME/data/pg_hba.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e host replication all 0.0.0.0/0 trust >> $PGHOME/data/pg_hba.conf 2>&1"

echo -e "host all all ::1/128 trust" >> $PGHOME/data/pg_hba.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e host all all ::1/128 trust >> $PGHOME/data/pg_hba.conf 2>&1"

echo -e "Modify the file $PGHOME/data/pg_hba.conf\t.............ok"

echo -e "logging_collector = on" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e logging_collector = on >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "log_directory = 'hgdb_log'" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e log_directory = 'hgdb_log' >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "log_filename = 'hgdb_double-%Y-%m-%d_%H%M%S.log'" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e log_filename = 'hgdb_double-%Y-%m-%d_%H%M%S.log' >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "listen_addresses = '*'" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e listen_addresses = '*' >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "wal_level = 'hot_standby'" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e wal_level = 'hot_standby' >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "max_wal_senders = 2" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e max_wal_senders = 2 >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "wal_keep_segments = 500" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e wal_keep_segments = 500 >> $PGHOME/data/postgresql.conf 2>&1"

#echo -e "synchronous_commit = on" >> $PGHOME/data/postgresql.conf 2>&1
#output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e synchronous_commit = on >> $PGHOME/data/postgresql.conf 2>&1"

#echo -e "synchronous_standby_names = 'standby_name'" >> $PGHOME/data/postgresql.conf 2>&1
#output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e synchronous_standby_names = 'standby_name' >> $PGHOME/data/postgresql.conf 2>&1"
 
echo -e "hot_standby = on" >> $PGHOME/data/postgresql.conf 2>&1
output_log $DOUBLE_ASYNC_DIR/log/log.log "echo -e hot_standby = on >> $PGHOME/data/postgresql.conf 2>&1"

echo -e "Modify the file $PGHOME/data/postgresql.conf\t.............ok"

#启动数据库服务
echo -e "Start the HighGo Databse service\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "$PGHOME/bin/pg_ctl start -w -D $PGHOME/data"
dbservice_check_02

#进行一次基础备份
echo -e "Start a base backup\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "$SCR_SQL_DIR/base_start.sh"
command_returnid_check $command_return_id "select pg_start_backup()"

#备份data到备库和double项目下
echo -e "Copy base data to stnadby server\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "scp -rv $PGHOME/data $STA_SYSTEM_USER@$STADB_HOST:$STADB_HOME/"
ssh $STA_SYSTEM_USER@$STADB_HOST "$STADB_HOME/double_check/check_dir_sta.sh $STADB_HOME/data"
echo -e "Backup the directory data\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "cp -rv $PGHOME/data $DOUBLE_ASYNC_DIR/data_backup/data_base"
check_dir $DOUBLE_ASYNC_DIR/data_backup/data_base

output_log $DOUBLE_ASYNC_DIR/log/log.log "$SCR_SQL_DIR/base_stop.sh"
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "scp -v $SCR_SQL_DIR/recovery_async.conf $STA_SYSTEM_USER@$STADB_HOST:$STADB_HOME/data/recovery.conf"
ssh $STA_SYSTEM_USER@$STADB_HOST "$STADB_HOME/double_check/check_file_sta.sh $STADB_HOME/data/recovery.conf"
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh -t $STA_SYSTEM_USER@$STADB_HOST rm -rf $STADB_HOME/data/postmaster.pid"

#启动备库
echo -e "Start the stnadby server HighGo Database servce\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh -t $STA_SYSTEM_USER@$STADB_HOST $STADB_HOME/bin/pg_ctl start -w -D $STADB_HOME/data"
...
do
  ...
  if [[ $master_wal_stat == "async" ]]
    then
      output_important_text "The double-click Hot standby is asynchronous replication\t.............ok"
      break
    else
      output_important_text "This double-click Hot sparing is not a asynchronous copy,please check\t.............no"
      sleep 2
  fi
  continue
done
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "$PGHOME/bin/pgbench -h $PGHOST -p $PGPORT -U $PGUSER -i -s 10 $PGDATABASE"
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "$PGHOME/bin/pg_ctl stop -w -D $PGHOME/data"
dbservice_check_03

#touch触发文件，备库升主库
echo -e "Create switch trigger file in the Standby server\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh $STA_SYSTEM_USER@$STADB_HOST touch $STADB_HOME/trigger_file"
sleep 10
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "cp $SCR_SQL_DIR/recovery_async_master.conf $PGHOME/data/recovery.conf"
...
#主库使用pgbench初始化数据
echo -e "Importing test data using pgbench\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "$PGHOME/bin/pgbench -h $STADB_HOST -p $STADB_PORT -U $STADB_USER -i -s 1 $STADB_DATABASE"
...
output_important_text "The Double-click Hot Standby backup(async) test success\t.............ok"

#停止主备库数据库服务
echo -e "Stop the Standby(old master) service\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "$PGHOME/bin/pg_ctl stop -w -D $PGHOME/data -m fast "
dbservice_check_03

echo -e "Stop the Master(old standby) service\t............."
output_log $DOUBLE_ASYNC_DIR/log/log.log "ssh $STA_SYSTEM_USER@$STADB_HOST $STADB_HOME/bin/pg_ctl stop -w -D $STADB_HOME/data -m fast"
...
output_log $DOUBLE_ASYNC_DIR/log/log.log "cp -rv $PGHOME/data $DOUBLE_ASYNC_DIR/data_backup/data_master"
check_dir $DOUBLE_ASYNC_DIR/data_backup/data_master
echo -e "Backup the last Master server data to directory $DOUBLE_ASYNC_DIR/data_backup/data_master\t.............ok"

output_log $DOUBLE_ASYNC_DIR/log/log.log "scp -rv $STA_SYSTEM_USER@$STADB_HOST:$STADB_HOME/data $DOUBLE_ASYNC_DIR/data_backup/data_standby"
check_dir $DOUBLE_ASYNC_DIR/data_backup/data_standby
echo -e "Backup the last Standby server data to directory $DOUBLE_ASYNC_DIR/data_backup/data_standby\t.............ok"

echo -e "$encry_cho $fdefenji double_wal_async : ok" >> $LOG_DIR/log.log
